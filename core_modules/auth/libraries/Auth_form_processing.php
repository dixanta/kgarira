...
class Auth_form_processing
{
	...
	function logout()
	{
		...
		if($this->CI->preference->item('activate_facebook')){		
			if($this->CI->facebook->getUser())
			{
				redirect(site_url('home/facebook_logout'));
			}		
		}
	...
	function _register()
	{
		...
		/*$slug['slug_name'] = $this->CI->input->post('username');
		$slug['route']='user/index/'.$data['user_profiles']['user_id'];
		$this->CI->slug_model->insert('SLUG',$slug);
		*/
		//insert into user table
		//$data['user_profiles']['slug_id'] = $this->CI->db->insert_id();
	...
	{
		try{
			if($this->CI->facebookUser){
				$this->CI->facebookUser=$this->CI->facebook->api('/me');
				$this->scrap_facebook_data();
			}
			else
			{
				$this->CI->session->sess_destroy();				
			...
			$this->CI->facebookUser=NULL;
			session_destroy();
		}		
	}

	/*private function scrap_facebook_data()
	{
		if(!$this->CI->session->userdata('id'))
		{
			//echo '<pre>';
			$facebook=$this->CI->facebookUser;
			if(!empty($facebook['username']))
			{
				$this->CI->db->or_where('username',$facebook['username']);
			}
			if(!empty($facebook['email']))
			{
				$this->CI->db->or_where('email',$facebook['email']);
			}			
			$this->CI->db->or_where('username',$facebook['first_name'] . ' '.$facebook['last_name']);
			$user=$this->CI->user_model->getUsers(array('fb_id'=>$facebook['id']));

			if($user->num_rows()==0)
			{
				//print_r($this->facebookUser);
				$user_data=$this->get_facebook_user_data();
				$this->CI->user_model->insert('Users',$user_data);
				$profile_data=$this->get_facebook_profile_data();
				$profile_data['user_id'] = $this->CI->db->insert_id();
				$this->CI->user_model->insert('UserProfiles',$profile_data);
				$this->CI->user_id=$profile_data['user_id'];
			...
			$this->CI->userlib->set_userlogin($this->CI->user_id);
	...
	private function get_facebook_user_data()
	{
		$facebook=$this->CI->facebookUser;
		$uname=(!empty($facebook['username'])?$facebook['username']:$facebook['first_name'] . ' '.$facebook['last_name']);
		$data['username']=$uname;
	...
	/*private function get_facebook_profile_data()
	{
		$facebook=$this->CI->facebookUser;
		$data['firstname']=$facebook['first_name'];
		$data['lastname']=$facebook['last_name'];
